...
$(document).ready(function() {
        ...
        $(".new_branding iframe").removeAttr("width");
...
function positionHeadMen() {
	if($("#h1_title_wrap").length>0) {
		//console.log("asd");
	    var h1_el = $("#h1_title_wrap");
	    var positionSet = h1_el.offset().top;
	    $('.h1_title_abs').css('top', positionSet+'px');
	    $('.h1_title_abs').css('left', h1_el.offset().left+'px');
	    
	    if($('.h1_title_abs').length>0) {
		   if(!($('.h1_title_abs').is(":visible"))) {
...
var clearMeIntervalHead = setInterval(function(){
	positionHeadMen();
},500);
...
setTimeout(function(){
    ...
    //var banner1 = $('#header_banner1').height();
...
setTimeout(function(){
	positionHeadMen();
	clearInterval(clearMeIntervalHead);
}, 8000);
...
function setBranding(){
    var src = jQuery('#new_branding img').attr('src');
    if(typeof src !== 'undefined'){
        var n = src.indexOf("/www/delivery/");
    }

    if (src){
        if (n>0){
            jQuery('#new_branding a').css('background-image', 'url("' + src + '")');
        }
    }
}